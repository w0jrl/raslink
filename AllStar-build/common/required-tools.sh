...
apt-get -qq --fix-missing install -y ntpdate g++-4.8  g++ make build-essential automake git\
...
if [[ $release = "stretch" ]]; then
  apt-get -qq --fix-missing install -y php-cli php-curl
else
  apt-get -qq --fix-missing install -y php5-cli php5-curl
fi
if [[ $distro = "Raspbian" ]]; then
  apt-get -qq --fix-missing install -y i2c-tools libi2c-dev raspberrypi-kernel raspberrypi-kernel-headers fake-hwclock
  dpkg-reconfigure perl
else
  apt-get -qq --fix-missing install -y linux-headers-$(uname -r)
fi
